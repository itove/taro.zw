...
function Index() {
  ...
  useEffect(() => {
    Taro.request({
      url: Env.apiUrl + 'nodes/shangchao'
    })
    .then(res => {
      const data = res.data
      console.log(res)

      setJingList(data.nodes.map((node, index) => <SwiperItem1 node={node} index={index} type={0} />))
      setYouList(data.nodes.map((node, index) => <ListItem node={node} index={index} type={0} />))
  ...
  return (
    <View className="home">
      <View className="pagetitle">商超</View>
